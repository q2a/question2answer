...
class qa_html_theme extends qa_html_theme_base
{
    ...
    public function nav_main_sub()
    {
        ...
        // If user is admin and above add Opening tags for sub nav toggle
        if ( qa_get_logged_in_level() > QA_USER_LEVEL_MODERATOR ) {
            $this->output( '<div class="qam-nav-sub-wrapper">' );
            $this->output( '<div class="qam-nav-sub-toggle"></div>' );
        }

        $this->nav( 'sub' );

        // If user is admin and above add Closing tag for sub nav toggle
        if ( qa_get_logged_in_level() > QA_USER_LEVEL_MODERATOR ) {
            $this->output( '</div> <!-- END qam-nav-sub-wrapper -->' );
        }
    }
    ...
    private function qam_user_account()
    {
        if ( qa_is_logged_in() ) {
            ...
            if ( QA_FINAL_EXTERNAL_USERS ) {
                $tobar_avatar = qa_get_external_avatar_html( qa_get_logged_in_user_field( 'userid' ),

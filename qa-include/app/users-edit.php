...
/*
 * TODO: // add qa_create_new_user($uid, $email, $password, $handle, $level=QA_USER_LEVEL_BASIC, $confirmed=false)
 */

	function qa_create_new_user_with_id($uid, $email, $password, $handle)
/*
...
*/
	{
		...
		qa_db_user_create_with_id($uid, $email, $password, $handle, $level, qa_remote_ip_address());
		qa_db_points_update_ifuser($uid, null);
		qa_db_uapprovecount_update();

		if ($confirmed)
			qa_db_user_set_flag($uid, QA_USER_FLAGS_EMAIL_CONFIRMED, true);

		if (qa_opt('show_notice_welcome'))
			qa_db_user_set_flag($uid, QA_USER_FLAGS_WELCOME_NOTICE, true);
		...
		if (qa_opt('confirm_user_emails') && ($level<QA_USER_LEVEL_EXPERT) && !$confirmed) {
			$confirm=strtr(qa_lang('emails/welcome_confirm'), array(
				'^url' => qa_get_new_confirm_url($uid, $handle)
			));

			if (qa_opt('confirm_user_required'))
				qa_db_user_set_flag($uid, QA_USER_FLAGS_MUST_CONFIRM, true);

		} else
			$confirm='';

		if (qa_opt('moderate_users') && qa_opt('approve_user_required') && ($level<QA_USER_LEVEL_EXPERT))
			qa_db_user_set_flag($uid, QA_USER_FLAGS_MUST_APPROVE, true);

		qa_send_notification($uid, $email, $handle, qa_lang('emails/welcome_subject'), qa_lang('emails/welcome_body'), array(
		...
		qa_report_event('u_register', $uid, $handle, qa_cookie_get(), array(
			'email' => $email,
			'level' => $level,
		));
